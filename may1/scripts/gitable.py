...
def dumpSingleCommitStats(u, sha, token):
	url = u.split('?')
	u = url[0]
	u = u + '/' + sha
...
def dumpCommit1(u,commits,token):
	...
	for commit in w:
		...
		additions, deletions = dumpSingleCommitStats(u, sha, token)
		commitObj = L(sha = sha,
								user = user,
								time = time,
								message = message,
...
def launchDump():
	...
	conn.execute('''CREATE TABLE IF NOT EXISTS commits(id INTEGER NOT NULL, time DATETIME NOT NULL, sha VARCHAR(128),
				user VARCHAR(128), message VARCHAR(128), additions INTEGER, deletions INTEGER,
	...
	for commit in commits:
		if not commit.user in nameMap:
			...
			nameNum+=1
		commitTuples.append([commit.time, commit.sha, nameMap[commit.user], commit.message, commit.additions, commit.deletions])
	...
	try:
		...
		if len(commitTuples) > 0:
			conn.executemany('INSERT INTO commits (time, sha, user, message, additions, deletions) VALUES (?,?,?,?,?,?)', commitTuples)
			conn.commit()
